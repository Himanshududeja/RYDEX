            ...
            <div className='relative w-full max-w-md rounded-3xl bg-white border border-black/10 shadow-[0_40px_100px_rgba(0,0,0,0.35)] p-6 sm:p-8 text-black'>
              <div className='absolute right-4 top-4 text-gray-500 hover:text-black transition' onClick={onClose}>
                ...
                <div className='flex items-center gap-4 my-6'>
                  ...
                  <div className='flex-xs text-gray-500'> OR</div>
                  <div className='flex-1 h-px bg-black/10'/>
                </div>
                <div>
                  {step=="login" &&(
                    ...
                    >
                      ...
                      <div className='mt-5 space-y-4'>
                        <div className='flex items-center gap-3 border border-black/20 rounded-xl px-4 py-3'>
                          ...
                          <input type="email" placeholder='EMAIL' className='bg-transparent w-full outline-none text-sm' onChange={(e)=>setEmail(e.target.value)} value={email}/>
                        </div>
                        <div className='flex items-center gap-3 border border-black/20 rounded-xl px-4 py-3'>
                          <Lock size={18} className='text-gray-500'/>
                          <input type="password" placeholder='PASSWORD' className='bg-transparent w-full outline-none text-sm' onChange={(e)=>setPassword(e.target.value)} value={password}/>
                        </div>
                      ...
                      <p className='mt-6 text-center text-sm text-gray-500'>Don't have an account? <span onClick={()=>setStep
                      ("signup")}className='text-black font-medium hover:underline'>Sign Up</span></p>
                    </motion.div>
                  )}
                  {step=="signup" &&(
                    <motion.div
                    ...
                    >
                      ...
                      <div className='mt-5 space-y-4'>
                         <div className='flex items-center gap-3 border border-black/20 rounded-xl px-4 py-3'>
                          ...
                          <input type="text" placeholder='NAME' className='bg-transparent w-full outline-none text-sm' onChange={(e)=>setName(e.target.value)} value={name}/>
                        </div>
                        <div className='flex items-center gap-3 border border-black/20 rounded-xl px-4 py-3'>
                          <Mail size={18} className='text-gray-500'/>
                          <input type="email" placeholder='EMAIL' className='bg-transparent w-full outline-none text-sm' onChange={(e)=>setEmail(e.target.value)} value={email}/>
                      ...
                      <p className='mt-6 text-center text-sm text-gray-500'>Don't have an account? <span onClick={()=>setStep
                      ("login")}className='text-black font-medium hover:underline'>Login</span></p>
                    </motion.div>
                  )}
                  {step=="otp" &&(
                    <motion.div key="otp"
                    ...
                    >
                      ...
                      {error && <p className='text-red-500 text-sm'>*{error}</p>}
                      <button className='mt-6 w-full h-11 rounded-xl bg-black text-white font-semibold hover:bg-gray-900 transition flex justify-center items-center' onClick={handleVerifyEmail} disabled={loading}>
